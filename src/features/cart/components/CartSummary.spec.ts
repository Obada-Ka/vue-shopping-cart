...
describe('CartSummary', () => {
  ...
  it('disables checkout when shipping is 0', () => {
    const wrapper = createWrapper({
      ...DEFAULT_PROPS,
      shipping: 0
    })
  ...
  it('enables checkout when both subtotal and shipping are non-zero', () => {
    const wrapper = createWrapper()

    expect(wrapper.findComponent(BaseButtonStub).props('disabled')).toBe(false)
  })

  it('shows a success toast when checkout is clicked', async () => {
    const wrapper = createWrapper()

    await wrapper.find('.base-button-stub').trigger('click')

    expect(toastSuccessMock).toHaveBeenCalledTimes(1)
    expect(toastSuccessMock).toHaveBeenCalledWith('Proceeding to checkout!')
  })
  ...
  it('does not show a toast when checkout is disabled', async () => {
    const wrapper = createWrapper({
      ...DEFAULT_PROPS,
      subtotal: 0
    })
